...
# Simplified OEM price book (INR) — production would use a pricing service
OEM_PARTS = {
...
class CostEstimator:
    def estimate(self, *, severity: str, damages: List[Dict[str, Any]], make: str = "Generic") -> CostResult:
        ...
        for dmg in damages:
            ...
            if part in seen_parts:
                # Multiple damages on same part: add labour only
                book = OEM_PARTS.get(part, OEM_PARTS["bumper"])
            ...
            paint = book["paint"] * mult if dmg.get("type") in {"scratch", "paint_damage", "dent", "bent_metal"} else 0
            ...
            line_items.append(
                {
                    ...
                    "category": "parts",
                    "oem": True,
                    "amount": round(part_cost, 2),
                }
            )
            line_items.append(
                {
                    "description": f"Labour — {part}",
                    "category": "labour",
                    "hours": book["labour_hours"],
                    "rate": LABOUR_RATE,
                    "amount": round(labour, 2),
